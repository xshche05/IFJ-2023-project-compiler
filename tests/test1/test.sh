...
execTest () {
	echo -e "\e[33m--------------------------------\e[0m"
	bash -c "$compilerPath < $2 > tmp_output.txt"
	returnCode=$?
...
execTest "Eearly return from procedure" "input/proc_return.swift" "output/empty.txt" 7
execTest "Try to return value from procedure" "input/proc_return_with_type.swift" "output/empty.txt" 4
...
execTest "Non null lhs in coalescence" "input/coal_lhs_non_null.swift" "output/empty.txt" 0
