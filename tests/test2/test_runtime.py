...
def test(input_code: str, expected_output: str, program_stdin: str = "",
    ...
    def print_compiler_info():
        ...
        print_red(f"compiler returned error code {compiler_return_code} ({compiler_output.returncode})")

    # check expected compiler return code
    if expected_return_code != compiler_return_code:
        print_error(f"Error: compiler return code expected: {expected_return_code} got: {compiler_return_code}")
        print_compiler_info()
        nr_failed += 1
        return
